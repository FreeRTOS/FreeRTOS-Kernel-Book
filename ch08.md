...
Note: The basic priority inheritance mechanism implemented for FreeRTOS
mutexes has been designed with both space and execution cycles in
mind. A full-fledged priority inheritance mechanism requires
significantly more data and processor cycles to determine inherited
priority at any moment. The priority inheritance mechanism of FreeRTOS
implements several heuristics to minimize size increases to the kernel.
With these tradeoffs the simplified priority inheritance mechanism
works most like a full-fledged mechanism in single mutex inversions.

Most importantly - the simplified priority
