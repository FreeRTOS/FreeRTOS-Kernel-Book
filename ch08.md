...
- Function Reentrancy
  ...
  *Note: A new macro `configUSE_NEWLIB_REENTRANT` is added for reentrant functions. If configUSE_NEWLIB_REENTRANT is set to `1` in FreeRTOSConfig.h, then a newlib reent structure will be allocated for each created task.*
