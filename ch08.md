...
- Function Reentrancy
  ...
  If an application uses newlib C Library, it must set ```configUSE_NEWLIB_REENTRANT``` to 1
  in FreeRTOSConfig.h to ensure that the Thread Local Storage required by newlib is
  allocated correctly.

  If an application uses picolibc C Library, it must set ```configUSE_PICOLIBC_TLS``` to 1 in
