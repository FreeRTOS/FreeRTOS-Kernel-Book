...
“basic” because it has been designed with both space and execution
cycles in mind. A full priority inheritance mechanism requires
significantly more data and processor cycles to determine inherited
priority at any moment, especially when a task holds more than one mutex
at a time.
...
* A task can have an already inherited priority raised further if it
  takes a mutex without first releasing mutexes it already holds. When
  this is case, the task remains at the highest inherited priority until
  it has released all the mutexes it holds. That is true regardless of
  the order it releases the mutexes.
* A task will remain at the highest inherited priority if multiple
  mutexes are held regardless of any tasks waiting on any of the held
