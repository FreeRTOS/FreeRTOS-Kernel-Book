...
1. Set `configUSE_TIMERS` to 1 in FreeRTOSConfig.h.
2. Define the constants detailed in the table below in the applications FreeRTOSConfig.h header file
    2.1. configUSE_TIMERS                : Set configUSE_TIMERS to 1 in FreeRTOSConfig.h.
    2.2. configTIMER_TASK_PRIORITY       : Sets the priority of the timer service task between 0 and
                                           ( configMAX_PRIORITIES - 1 ).
    2.3. configTIMER_QUEUE_LENGTH.       : Sets the maximum number of unprocessed commands that the
                                           timer command queue can hold at any one time.
    2.4. configTIMER_TASK_STACK_DEPTH    : Sets the size of the stack (in words, not bytes) allocated to the
                                           timer service task.
...
```
* * *
*Listing 6.2. The software timer callback function prototype*
...
```
* * *
*Listing 6.2. The xTimerCreate() API function prototype*
...
```
* * *
*Listing 6.3. The xTimerStart() API function prototype*
...
- Return value
  ...
  - `pdFALSE`
    ...
    `pdFALSE` will be returned if the 'start a timer' command could not be
...
```
* * *
*Listing 6.4. Creating and starting the timers used in Example 6.1*
...
shown in Listing 6.5. The implementation of the auto-reload timer
callback function is shown in Listing 6.6.
...
```
* * *
*Listing 6.5. The callback function used by the one-shot timer in Example 6.1*
...
```
* * *
*Listing 6.6. The callback function used by the auto-reload timer in Example 6.1*
...
Listing 6.4), and the one-shot timer's callback function executing only
...
```
* * *
*Listing 6.7. The vTimerSetTimerID() API function prototype*
...
```
* * *
*Listing 6.8. The pvTimerGetTimerID() API function prototype*
...
timers are created. This difference is shown in Listing 6.9, where
...
```
* * *
*Listing 6.9. Creating the timers used in Example 6.2*
...
The implementation of `prvTimerCallback()` is shown in Listing 6.8.
...
```
* * *
*Listing 6.10. The timer callback function used in Example 6.2*
...
```
* * *
*Listing 6.11. The xTimerChangePeriod() API function prototype*
...
- Returned value
  ...
  - `pdFALSE`
    ...
    `pdFALSE` will be returned if the 'change period' command could not be
    written to the timer command queue because the queue was already
    full.
...
Listing 6.12 shows how the FreeRTOS examples that include self-checking
...
```
* * *
*Listing 6.12. Using xTimerChangePeriod()*
...
```
* * *
*Listing 6.13. The xTimerReset() API function prototype*
...
- Returned value
  ...
  - `pdPASS`
    ...
    `pdFALSE` will be returned if the 'reset' command could not be written
...
The software timer callback function is shown in Listing 6.14.
...
```
* * *
*Listing 6.14. The callback function for the one-shot timer used in Example 6.3*

Example 6.3 creates a task to poll the keyboard<sup>11</sup>. The task is shown
in Listing 6.15, but for the reasons described in the next paragraph,
Listing 6.15 is not intended to be representative of an optimal design.
...
technique had to be used instead. If Listing 6.15 was an interrupt service
...
```
* * *
*Listing 6.15. The task used to reset the software timer in Example 6.3*
