...
Tasks are implemented as C functions. Tasks nust implement the expected function
prototype which must return void and take a void pointer parameter, as shown in
Listing 4.1.
...
tasks, each created task is a separate execution instance. Each instance has its
own stack and thus its own copy of any automatic (stack) variables defined
...
stateDiagram
    direction LR
    NotRunningState: Not Running
    RunningState: Running

    NotRunningState --> RunningState
    RunningState --> NotRunningState

    direction TB

    note left of NotRunningState
        All tasks that are not currently Running
        are in the Not Running state.
    end note

    note right of RunningState
        Only one task can be in the
...
execute with limited access to the system's memory space. API functions without
"Restricted" in their names create tasks that execute in "privileged mode" and
have access to the system's entire memory space.

FreeRTOS ports that support Symmetric Multi Processing (SMP) allow different tasks
to run simulatenously on multiple cores of the same CPU. For these ports, you may
...
- `usStackDepth`
  ...
  to `unsigned long` or `size_t` in `FreeRTOSConfig.h` if the stack depth multiplied by
...
- `uxPriority`
  ...
  `(configMAX_PRIORITIES – 1)` is the highest priority. [Section 4.6](#46-task-priorities)
...
***
*Figure 4.2. The output produced when executing Example 4.1<sup>4</sup>*
...
int main( void )
{
     ...
     * (described later in this book). Chapter 3 provides more information on
     * heap memory management. Section XXX describes how to create the idle
     * and timer tasks using pre-allocated memory.
     */
    for( ;; );
}
...
The output from Example 4.2 is exactly as per that shown for example 1 in Figure 4.2.X
...
### Architecture-Optimised Schduler
...
Figure 4.7 expands on the simplified state diagram to include all od the
...
```{mermaid}
graph TB
  RunningState ---->|"VTaskSuspend() called"| SuspendedState
  RunningState ===> ReadyState
  ReadyState ===> RunningState

  SuspendedState ---->|"vTaskResume() Called"| ReadyState
  ReadyState --->|"vTaskSuspend() Called"| SuspendedState


  RunningState --->|"Blocking API Called"| BlockedState
  BlockedState -->|"Event Occurred"| ReadyState
  BlockedState --->|"vTaskSuspend() Called"| SuspendedState

  subgraph Not Running
...
Figure 4.11 shows the output produced by Example 4.6, with an explanation of the observed behaviour
given by the execution sequence shown in Figure 4.12.

![](media/image19.jpg)   *Figure 4.11. The output produced when Example 4.6 is executed*

![Figure 4.12. The execution pattern of Example 4.6](media/image20.png)
...
### 4.13.2 Selecting the Scheduling Algorithm
...
### 4.13.3 Prioritized Preemptive Scheduling with Time Slicing
...
- Preemptive
  ...
  Preemptive scheduling algorithms will immediately 'preempt' the *Running* state task if a task that
  has a priority higher than the *Running* state task enters the Ready state. Being preempted means
  being involuntarily moved out of the *Running* state and into the Ready state (without explicitly
  yielding or blocking) to allow a different task to enter the *Running* state. Task preemption can
...
![Execution pattern highlighting task prioritization and preemption in a hypothetical application in which each task has been assigned a unique priority](media/image26.png)

*Figure 4.18. Execution pattern highlighting task prioritization and preemption
 in a hypothetical application in which each task has been assigned a unique priority*
...
![Execution pattern highlighting task prioritization and time slicing in a
  hypothetical application in which two tasks run at the same priority](media/image27.png)
...
- The Idle Task and Task 2
  ...
  starts on each tick interrupt, which in Figure 4.19 is at times t1, t2,
...
![The execution pattern for the same scenario as shown in Figure 4.19, with
 configIDLE_SHOULD_YIELD set to 1](media/image28.png)
*Figure 4.20 The execution pattern for the same scenario as shown in Figure 4.19
 with `configIDLE_SHOULD_YIELD` set to 1*
...
### 4.13.4 Prioritized Preemptive Scheduling without Time Slicing
...
![Execution pattern that demonstrates how tasks of equal priority can receive hugely
 different amounts of processing time when time slicing is not used](media/image29.png)
...
Figure 4.22 demonstrates the behaviour of the co-operative scheduler. The
...
- When using the preemptive scheduler the *Running* state task can be
