...
observed behaviour given by the execution sequence shown in Figure 4.12.
...
void vTask1( void * pvParameters )
{
    ...
    for( ;; )
    {
         ...
         * Pass the addres of xTask2Handle as the pxCreatedTask parameter so
...
## 4.12 Thread Local Storage and Reentrancy
...
### 4.12.1 C Runtime Thread Local Storage Implementations

Most embedded libc implementations provide APIs to ensure that non-reentrant
functions can work correctly in a multithreaded environment. FreeRTOS includes
support for the commonly used open-source Newlibc and Picolibc libraries. These pre-built c runtime
Thread Local Storage implementations can be enabled by defining either of the 
following macros respectively:

- ```configUSE_NEWLIB_REENTRANT```
- ```configUSE_PICOLIBC_TLS```

### 4.12.2 Custom C Runtime Thread Local Storage
...
### 4.12.3 Application Thread Local Storage

In addition to C Runtime Thread Local Storage, application developers may also
define a set of thread-specific pointers to be 
...
Figure 4.22 demonstrates the behaviour of the cooperative scheduler. The
horizontal dashed lines in Figure 4.22 show when a task is in the Ready
state.

***
![](media/image30.png)  
*Figure 4.22 Execution pattern demonstrating the behaviour of the cooperative scheduler*
***
