...
A queue can hold a finite number of fixed size data items<sup>8</sup>. The
...
FreeRTOS uses the queue by copy method because it is simultaneously more
...
- Memory protected systems restrict access to RAM, in which cases
...
Queues are objects in their own right that can be accessed by any task
...
`xQueueReset()` is an API function that returns a previously created queue
...
receiving task. Table 22 provides further explanation of Figure 5.6, and
...
- t2
  ...
  task that can run, so enters the Running state.

- t3

  Task Sender 2 finds the queue is already full, so enters the Blocked
  state to wait for its first send to complete. Task Receiver is now the
  highest priority task that can run, so enters the Running state.
...
### 5.5.2 Using a Queue to Send Different Types and Lengths of Data<sup>9</sup>
...
different sizes, data of different meaning, and data from different
...
- Return Value
  ...
  1. `pdPASS`
      ...
      This indicates the queue set was created successfully.</p>
...
the queue referenced by the received handle contains data, so reads the
...
5.3, `xQueueSelectFromSet()` is called with an indefinite block time, so
...
`vSenderTask2()`, causing the strings sent by `vSenderTask1()` to print out
...
- A queue from which uint32\_t values are read.
...
queue may get described as a mailbox because of the way it is used in
...
Listing 5.28 shows a queue being created for use as a mailbox.
...
Listing 5.30 shows xQueueOverwrite() being used to write to the mailbox
