...
FreeRTOS applications are typically structured as a series of independent tasks that communicate to collectively provide the system functionality.  Task notifications are an efficient mechanism allowing one task to directly notify another task.
...
`configTASK_NOTIFICATION_ARRAY_ENTRIES` * 8 bytes per task with the
default value being 1 or 8 bytes per task.
...
>>>>>>> 9cd0d37 (some changes to chapter 10)
...
>Note: There are only ISR functions for sending a notification.  Interrupts do not occur
