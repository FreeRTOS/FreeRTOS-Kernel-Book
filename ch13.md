...
- Different library implementations expect the priority of an
  interrupt to be specified in a different way. Again, particularly
...
- Different implementations of the same architecture implement a
  ...
  a Cortex-M processor from another manufacturers may implement 4
  priority bits.
...
- The bits that define the priority of an interrupt can be split
  ...
  specifying a pre-emption priority, so sub-priorities are not used.
...
stack related issues<sup>28</sup>.
...
`uxTaskGetStackHighWaterMark2()` API can be used for the same application,
with the difference being in their return type. Using configSTACK_DEPTH_TYPE     allows the user to decide the return type.
...
This method is to add ISR stack checking to task stack checking. A problem in the ISR stack will trigger an assert, not call the stack overflow hook function (because the stack overflow hook is specific to a task stack, not the ISR stack).

## 13.4 Cautious Use of printf() and sprintf()
...
To be able to add more tasks, either increase the heap size, or remove
some of the existing demo tasks. When increasing the heap size to run tasks simultaneously, keep in mind the amount of RAM available on the platform, depending on that, there will be a threshold on the number of simultaneous tasks that can be run on the platform. See section 3.2, Example Memory
...
re-enable again until the first task starts to execute. This is done to
protect the system from crashes caused by interrupts attempting to use
...
inside a critical section.
 ...
 ## 13.6 Good Debugging Practices

In addition to the common causes for any FreeRTOS application to fail as described above, users can encounter other issues. In such scenarios , the following debugging practices allow the user or the FreeRTOS community to get a better picture of the issue to arrive at a solution .

- Have configASSERT() defined and stack checking enabled in the application configuration file .
- Put break-points at the entry point of each task of the application, to make sure all the task creation were successful. If Task creation fails, check the heap requirement.
- Make sure the configMAX_SYSCALL_INTERRUPT_PRIORITY is set appropriately , based on the platform as discussed in section 6.8 and 12.2 .
- If some tasks face starvation issue, check if the scheduling parameters like configUSE_TIME_SLICING  are being used as per design ( section 3.12 )
- In case of hard fault, if all stacks are good and heap is large enough, this page (https://www.freertos.org/Debugging-Hard-Faults-On-Cortex-M-Microcontrollers.html) can provide some hint.
- If memory leak is suspected, after setting up a debugger , check the logging tasks properly, if the allocated heap was freed after use.
