...
```
Note: `configRUN_TIME_COUNTER_TYPE` defaults to uint32_t for backward compatibility, but can be overridden in FreeRTOSConfig.h 
if `uint32_t` is too restrictive.
* * *
